...
const PatientList = () => {
  ...
  const [editData, setEditData] = useState({ name: '', title: '', appointmentDateAndTime: '' });
  ...
  const handleOpenEditDialog = () => {
    if (selectedAppointment) {
      const formattedDate = new Date(selectedAppointment.appointmentDateAndTime).toISOString().slice(0, 16);
      setEditData({
  ...
  return (
    <>
      {/* Header Section */}
      <Box sx={{ backgroundColor: '#f5f5f5', padding: '16px', marginBottom: '20px' }}>
        <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
          <Box sx={{ display: 'flex', alignItems: 'center' }}>
            <PersonOutlineIcon sx={{ fontSize: '40px', color: 'blue', marginRight: '8px' }} />
        ...
        <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <Typography variant="body1" color="black">
            <Box component="span" sx={{ fontSize: '25px', fontWeight: 'bold', paddingLeft: '13px', color: 'blue' }}>
              {appointments.length}
            </Box>
            : Total Appointments
          </Typography>
          ...
          <TableBody>
            {appointments.map((appointment, index) => (
              <TableRow key={appointment.id}>
                <TableCell sx={{ color: 'black', fontWeight: '500' }}>{index + 1}</TableCell>
                <TableCell sx={{ color: 'black', fontWeight: '500' }}>{appointment.name}</TableCell>
                <TableCell sx={{ color: 'black', fontWeight: '500' }}>{appointment.title}</TableCell>
                <TableCell sx={{ color: 'black', fontWeight: '500' }}>
                  {new Date(appointment.appointmentDateAndTime).toLocaleDateString('en-US', {
                    weekday: 'long',
                    year: 'numeric',
                    month: 'long',
                    day: 'numeric',
                ...
                <TableCell sx={{ color: 'black', fontWeight: '500' }}>
                  {new Date(appointment.appointmentDateAndTime).toLocaleTimeString('en-US', {
                    hour: 'numeric',
                    minute: 'numeric',
                    second: 'numeric',
                    hour12: true,
                ...
                <TableCell>
                  ...
                  >
                    <MenuItem onClick={handleOpenEditDialog}>Edit Patient</MenuItem>
