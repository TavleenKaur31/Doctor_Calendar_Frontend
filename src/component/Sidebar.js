...
const Sidebar = () => {
  ...
  return (
    <StyledDrawer variant="permanent" anchor="left">
      ...
      <Box sx={{ padding: '16px', textAlign: 'center' }}>
        <Logo> 
          <img src={Logoo} alt="Logo" style={{ width: 200, height: 'auto', marginBottom: 16 }} />
        </Logo>
      </Box>

      {/* Main content */}
      <List sx={{ flexGrow: 1 }}> {/* Takes up remaining space */}
        {menuItems.map((item) => (
          <ListItem 
            button 
            key={item.text} 
  ...
  <Avatar alt="Drg. Adam H." src={DentistImage} sx={{ width: 56, height: 56, marginRight: 2 }} />
  <div>
    <Typography variant="subtitle1" fontWeight="bold">Drg. Adam H.</Typography>
    <Typography variant="body2" color="textSecondary">Dentist</Typography>
  </div>
</Footer>
